...
class RuneLiteBot(Bot, metaclass=ABCMeta):
    ...
    def potentially_relog_with_delayed_login(
        ...
        Args:
            prob (float, optional): Probability of logging out and relogging. Defaults to 1% chance.
            wait_time (int, optional): Time in seconds to wait before logging back in.

        Returns:
            bool: True if logout and relogin occurred, False otherwise.
        """
        prob_logout = abs(random.normalvariate(prob, prob / 10))
        if rd.random_chance(prob=prob_logout):
            ...
            self.login()
            self.log_msg("Logged back in after break.", overwrite=True)
            return True
        return False
